...
3.  **Frame Seeking:**
    ...
    *   If `--count` is used, exact timestamps are evenly spaced across the full duration (inclusive of the first and last frames, or the midpoint when only one capture is requested) and the corresponding frame numbers are derived from those timestamps.
