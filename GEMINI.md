...
Usage:
```bash
vid-scr <video-file> [--interval <seconds>] [--output <directory>] [--merge] [--size <pixels>] [--number <count>]
```
...
2.  **Duration and Frame Rate:** The video's duration and frame rate will be determined using properties of the `VideoCapture` object. This is essential for calculating screenshot intervals, especially when the `--number` option is used.
3.  **Frame Seeking:**
    ...
    *   If `--number` is used, the interval between screenshots will be calculated (`interval = video_duration / number`) and then the frame numbers to capture will be determined.
